...
const inventory = computed(() => inventoryStore.rightInventory)
...
const showWeight = computed(() => inventory.value.type !== 'shop' && inventory.value.type !== 'crafting')
</script>
  ...
  <div class="bg-slate-800/80 p-4 rounded-lg">
    ...
    <div class="mb-2 flex flex-col gap-1">
      <div class="flex justify-between items-center">
        ...
        <span v-if="showWeight" class="text-sm">
          {{ totalWeight?.toFixed(2) }}/{{ maxWeight?.toFixed(2) }}
        </span>
      </div>
      <WeightBar v-if="showWeight" :percent="weightPercent" />
    </div>
